...
onMounted(async () => {
    ...
    if(store.state.layoutStore.callHide){
        await store.dispatch("handleQueryMessageUser", toRaw({userid: getUser().user_id})).then((res) => {
            console.log(res);
            for(let i in res.accept){
                setMessageInnerHTML_Other(res.accept[i].notice_content,res.accept[i].notice_time);
            }
            for(let i in res.send){
                setMessageInnerHTML_Me_Succeed(res.send[i].notice_content,res.send[i].notice_time);
            }
        })
    }
})
