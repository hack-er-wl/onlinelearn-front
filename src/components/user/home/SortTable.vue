      ...
      <div class="main">
        <div class="sortTable">
          <div class="classes" v-for="(n,i) in store.state.layoutStore.fields" :key="i">
            <p style="width: 50px;color: gray;display:flex;align-items: center;justify-content:center;font-weight: bold;font-size: 13px">{{n.field_name}}</p>
            <el-divider style="height: auto" direction="vertical"/>
            <el-button v-for="(t,i) in n.classes" :key="i" style="font-size: 13px;color: gray" round @click="handleClick(n.field_name,t)">
              {{t.class_name}}
            </el-button>
          </div>
        </div>
